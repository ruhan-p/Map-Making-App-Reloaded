  ...
  async function installOnce() {
    ...
    function renderDropdownList(items, query) {
      ...
      const exactMatchExists = !!trimmedQuery && (
        (hasItems && items.some(it => it.name === trimmedQuery)) ||
        TAG_NAME_SET.has(trimmedQuery)
      );
    ...
    proxy.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') { dd.style.display = 'none'; proxy.blur(); }
        ...
        const hasExistingOptions = !!dd.querySelector('.ext-tag-dd__item:not([data-ext-new="1"])');
        if (newOption && !hasExistingOptions) {
          if (!trimmed) return;
          e.preventDefault();
          addNewTag(trimmed);
          return;
        ...
        editItem.addEventListener('click', async (e) => {
          ...
          try {
            ...
            try {
              ...
              if (typeof name === 'string' && name.trim() && name.trim() !== (initialName || '').trim()) {
                try {
                  window.__extHandleTagRename?.({ tagEl, oldName: initialName || '', newName: name.trim() });
                } catch {}
              }
            } catch {}
          } catch (err) {
