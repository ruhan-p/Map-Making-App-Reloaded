    ...
    try {
      ...
      if (resolved) {
        ...
        try {
          ...
          if (window.__extTextColorForBg) menu.style.color = window.__extTextColorForBg(baseColor);
        } catch (err) {
...
(() => {
  ...
  window.__extInitLayoutPopup = function __extInitLayoutPopup(btn, callbacks = {}) {
    ...
    if (!menuEl || !menuEl.isConnected) {
      ...
      inner.append(drag.btn, def.btn, res.btn);
    }
...
function getTagColor(el) {
  ...
  const bg = pickBg();
  const fg = (window.__extTextColorForBg) ? window.__extTextColorForBg(bg) : '#fff';
...
(() => {
  ...
  function waitSel(sel, { root = document, timeout = 20000, poll = 150 } = {}) {
    const start = performance.now();
    return new Promise((resolve, reject) => {
      (function tick() {
        const el = root.querySelector(sel);
        if (el) return resolve(el);
        if (performance.now() - start > timeout) return reject(new Error(`timeout: ${sel}`));
  ...
  function createProxyInput(realInput, host) {
    ...
    if (!proxy) {
      ...
      proxy.placeholder = realInput.getAttribute('placeholder') || 'Filter tags...';
  ...
  async function install() {
    const FILTER_INPUT_SEL = `${OVERVIEW_SEL} input[placeholder*="Filter tag" i], ${OVERVIEW_SEL} input[aria-label*="Filter" i]`;
    const realInput = await waitSel(FILTER_INPUT_SEL).catch(() => null);
    if (!realInput) return;
...
function OpenEditorPopup(shape, el, position) {
  (async () => {
    try {
      ...
      const newTextColor = (typeof __extContrastTextForRgbArray === 'function')
              ? __extContrastTextForRgbArray(newRgbArray)
              : (typeof __extTextColorForBg === 'function' ? __extTextColorForBg(newHexColor) : '');
      ...
      if (el) {
        ...
        try {
          ...
          } else if (window.__extTextColorForBg) {
            el.style.color = __extTextColorForBg(newHexColor);
          }
        } catch {}
      }
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                if (rgbArray && typeof __extContrastTextForRgbArray === 'function') {
                  tagEl.style.color = __extContrastTextForRgbArray(rgbArray);
                } else if (typeof __extTextColorForBg === 'function') {
                  tagEl.style.color = __extTextColorForBg(hex);
                } else if (window.__extTextColorForBg) {
                  tagEl.style.color = window.__extTextColorForBg(hex);
                }
              } catch {}
            } catch {}
          } catch (err) {
