...
const SELECTORS = {
  ...
  saveBtn: '[data-qa="map-save"].button--primary',
  modal: '.modal__dialog',
...
let __extColorProbeElement = null;

function __extEnsureColorProbe() {
  if (typeof document === 'undefined') {
    return null;
  }
  if (__extColorProbeElement && __extColorProbeElement.isConnected) {
    return __extColorProbeElement;
  }
  let probe = document.getElementById('__ext_color_probe');
  if (!probe) {
    try {
      probe = document.createElement('div');
      probe.id = '__ext_color_probe';
      probe.style.cssText = 'position:fixed;left:-100000px;top:-100000px;pointer-events:none;contain:paint style layout;';
      (document.body || document.documentElement)?.appendChild(probe);
    } catch {
      probe = null;
    }
  }
  __extColorProbeElement = probe || null;
  return __extColorProbeElement;
}

function __extNormalizeRgbInput(value) {
  if (Array.isArray(value) && value.length >= 3) {
    return value.slice(0, 3).map(n => Math.round(__extClampValue(n, 0, 255)));
  }
  ...
  const hex = __extCssColorToHex(trimmed);
  if (hex) {
    const rgb = __extHexToRgb(hex);
    return rgb ? [rgb.r, rgb.g, rgb.b] : null;
  }
  const probe = __extEnsureColorProbe();
  if (!probe || typeof window === 'undefined' || !window.getComputedStyle) {
    return null;
  }
  const previous = probe.style.color;
  try {
    probe.style.color = trimmed;
    const match = window.getComputedStyle(probe).color.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/i);
    if (match) {
      return [Number(match[1]), Number(match[2]), Number(match[3])].map(n => Math.round(__extClampValue(n, 0, 255)));
    }
  } catch {
    // ignore
  } finally {
...
function __extGetContrastTextColor(colorValue) {
  const rgb = __extNormalizeRgbInput(colorValue);
  if (!rgb) {
    return '#fff';
  }
